...
const EditServices = () => {
  ...
  const getServiceDetails = async () => {
    try {
      await axios.get(`api/admin/edit-service/${id}`).then((res) => {
        if (res.data.status === 200) {
          setFormData({
            id: res.data.service.id,
            serviceType: res.data.service.service_type,
            category: res.data.service.category_id,
            tokenAmount: res.data.service.token_money,
          });

          //console.log(res.data.service.category_id);

          setRemarks(res.data.service.remarks);

          let temp = [];

          res.data.informations.forEach((item, index) => {
            temp.push({
              id: informations.length + 1,
              particulars: item.particulars,
              type: item.type,
              class: item.class,
              mandatory: item.mandatory === 1 ? true : false,
              inform_id: item.id,
            });
          });
          ...
          setSubHeadLevel1(res.data.service_head_level_1);

          setSubHeadLevel2(res.data.service_head_level_2);
        }
      });
    } catch (error) {
      errorToast(error)
  ...
  const getAllDocuments = async () => {
    try {
      ...
      console.log('id= ' + id + ' type_name= ' + type_name);
      await axios
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            console.log(res.data.serviceid + ' ' + res.data.sr);
  ...
  const deleteInformation = async (index, id) => {
    ...
    try {
      await axios.get(`api/admin/delete-info-required/${id}`).then((res) => {
        if (res.data.status === 200) {
          getServiceDetails();
        }
      });
  ...
  const deleteServiceHeadLevel = async (id) => {
    try {
      await axios
        .get(`api/admin/delete-service-head-level/${id}`)
        .then((res) => {
          if (res.data.status === 200) {
            getServiceDetails();
          }
  ...
  const deleteSubHeadLevel1 = (index, level_id) => {
    const con = window.confirm('Are you sure ?');
    ...
    if (level_id > 0) {
      deleteServiceHeadLevel(level_id);
    }
  ...
  const deleteSubHeadLevel2 = (index, level_id) => {
    const con = window.confirm('Are you sure ?');
  ...
  return (
    <div>
      <div className="flex items-center mb-4">
        ...
        />
        <h1 className="md:text-lg font-semibold ml-3">Edit Services</h1>
      </div>
      <section>
        <form>
          {/* service details  */}
                ...
                >
                  ...
                  {allcategory.map((option, index) => (
                    <option
                      value={option?.id}
                      key={index}
                      selected={option.id === formData.category ? true : false}
                    >
          ...
          <div className="bg-white p-6 rounded-md mb-5">
                  ...
                  {subheadlevel2?.map((data, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 flex justify-between items-center">
                        <div
                          onClick={() =>
                            deleteSubHeadLevel2(`${index}`, `${data.level_id}`)
                          }
            ...
            <button
              ...
              type="button"
              className={`mt-3 ${subheadlevel2.length > 0 && 'ml-5'
                } text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-xs md:text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 focus:outline-none dark:focus:ring-green-800`}
            >
              Add more +
            </button>

            <h1 className="font-semibold md:text-lg">Information Required</h1>
            <hr className="my-3" />
                  ...
                  {informations?.map((data, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 flex justify-between items-center">
                        ...
                        <div
                          onClick={() =>
                            deleteInformation(`${index}`, `${data.inform_id}`)
                          }
                          className="bg-green-400 hover:bg-green-500 p-1 cursor-pointer"
                        >
                          <AiOutlineDelete size={20} className=" text-white" />
              ...
              {documents?.map((item, index) => (
                <div className="flex items-center justify-center" key={index}>
                  <input
                    ...
                    defaultChecked={item.select ? true : false}
                  />
